...
public class ReservasView extends JFrame {
	...
	public static JComboBox<MetodoDePago> txtFormaPago;
	int xMouse, yMouse;
	private JLabel labelExit;
	private JLabel lblValorSimbolo; 
	private JLabel labelAtras;
	public ReservaHuespedController reservaController = new ReservaHuespedController();
	int propertyChangeBypass = 0;
	...
	public ReservasView() {
		...
		lblValorSimbolo.setBounds(80, 332, 17, 25);
		...
		lblCheckIn.setBounds(68, 136, 215, 14);
		lblCheckIn.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		panel.add(lblCheckIn);
		
		JLabel lblCheckOut = new JLabel("Fecha de Check-out");
		lblCheckOut.setForeground(SystemColor.textInactiveText);
		lblCheckOut.setBounds(68, 221, 215, 14);
		...
		txtFechaS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				propertyChangeBypass++;
				if(propertyChangeBypass > 2) { // It won't count the previous unwanted propertyChanges.
					reservaController.calcularReserva(txtFechaE,txtFechaS,txtValor);
				}
			}
		});
		panel.add(txtFechaS);
		...
		txtValor.setBounds(95, 328, 43, 33);
		...
		lblValor.setBounds(68, 303, 215, 14);
		lblValor.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		panel.add(lblValor);
		
		txtFormaPago = new JComboBox<MetodoDePago>();
		txtFormaPago.setBounds(68, 417, 289, 38);
		txtFormaPago.setBackground(SystemColor.text);
		txtFormaPago.setBorder(new LineBorder(new Color(255, 255, 255), 1, true));
		txtFormaPago.setFont(new Font("Roboto", Font.PLAIN, 16));
		
		txtFormaPago.setModel(reservaController.getPaymentsModel());
		...
		lblTitulo.setBounds(99, 60, 240, 42);
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtFechaE.getDate() != null && ReservasView.txtFechaS.getDate() != null) {
					if(ReservasView.txtFechaE.getDate().before(ReservasView.txtFechaS.getDate())) {
						// Fechas y valor de la reserva
						CalculosReserva calculosReserva = new CalculosReserva();
						String fechaCheckInString = calculosReserva.getFechaCheckInString();
						String fechaCheckOutString = calculosReserva.getFechaCheckOutString();
						String txtPrecio = txtValor.getText();
						String metodoDePagoValor = txtFormaPago.getSelectedItem().toString();
						
						// Guarda los valores en reservaController para utilizarlos luego.
						reservaController.datosReservaHolder(fechaCheckInString,fechaCheckOutString,
								txtPrecio,metodoDePagoValor);
						
						dispose();
						RegistroHuesped registro = new RegistroHuesped();
						registro.setVisible(true);
					}else {
						JOptionPane.showMessageDialog(null, "Verifique las fechas.");
					}
				} else {
